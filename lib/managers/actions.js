...
const actions = module.exports = {
  play(loader, file, state) {
    return plugins.list('loaders')(state)
      ...
      .then(obj => λ.tryCatch(
        ...
        () => {
          const child = spawn(
            λ.head(obj.game.command),
            λ.tail(obj.game.command),
            { stdio: 'inherit', cwd: obj.game.location })

          return { game: obj.game, loader: obj.loader, child }
        },

        // spawn failed
        () => ({ game: obj.game, loader: obj.loader })))
  ...
  deploy(state) {
    return plugins.list('deployers')(state)
      ...
      .then(deployers => Promise.map(actions.search(state),
        game => Promise.map(deployers,
          deployer => plugins.deploy(game, deployer, state)))
